...
<div style="display: none" id="ccm-jobs-automation-dialogs">
    <?php foreach($installedJobs as $j) { ?>
    	<div id="jd<?php echo $j->getJobID()?>" class="ccm-ui">
    		<form action="<?php echo $view->action('update_job_schedule')?>" method="post" data-form="schedule-job">
    			...
    			<fieldset class="ccm-jobs-automation-schedule-cron" <?php echo ($j->isScheduled?'style="display: none;"':'')?>>
    				<div class="well">
    					<?php if ($j->supportsQueue()) { ?>
    						...
    						<div class="form-group">
    						    <textarea rows="2" class="ccm-default-jobs-url form-control"><?php echo BASE_URL . $view->url('/tools/required/jobs?auth=' . $auth . '&jID=' . $j->getJobID())?></textarea>
                            </div>
    						...
    						<div class="form-group">
    						    <textarea rows="2" class="ccm-default-jobs-url form-control"><?php echo BASE_URL . REL_DIR_FILES_TOOLS_REQUIRED . '/jobs/run_single?auth=' . $auth . '&jID=' . $j->getJobID()?></textarea>
                            </div>
                            
    						<p><?php echo t('Then, make sure this URL is scheduled to run frequently, like every 3-5 minutes:')?></p>
    						
    						<div class="form-group">
    						    <textarea rows="2" class="ccm-default-jobs-url form-control"><?php echo BASE_URL . REL_DIR_FILES_TOOLS_REQUIRED . '/jobs/check_queue?auth=' . $auth?></textarea>
    						</div>
    					...
    					<?php } else { ?>
    						...
    						<div>
    						    <textarea rows="2" class="ccm-default-jobs-url form-control"><?php echo BASE_URL . $view->url('/tools/required/jobs/run_single?auth=' . $auth . '&jID=' . $j->getJobID())?></textarea>
                            </div>
    					<?php } ?>	
    				</div>
    			</fieldset>
		...
		<?php foreach($availableJobs as $availableJobName => $job):?>
    		<tr> 
    			...
    			<td><?php if(!$job->invalid):?>
    				<a href="<?php echo $view->action('install', $job->jHandle)?>" class="btn btn-small pull-right"><?php echo t('Install')?></a>
    			<?php endif?></td>
    		</tr>	
		<?php endforeach?>
	</tbody>
...
<div class="well">
    ...
    <div>
        <input type="text" style="width: 700px" class="ccm-default-jobs-url" value="<?php echo BASE_URL . $view->url('/tools/required/jobs?auth=' . $auth)?>" />
    </div>
</div>
<?php } ?>
            ...
            <div class="well">
                ...
                <form action="<?php echo $view->action('update_set_schedule');?>" method="post">
                    ...
                    <fieldset class="ccm-jobs-automation-schedule-cron" <?php echo ($set->isScheduled?'style="display: none;"':'')?>>
                        ...
                        <div class="form-group">
                            <textarea rows="2" class="ccm-default-jobs-url form-control"><?php echo BASE_URL . $view->url('/tools/required/jobs?auth=' . $auth . '&jsID=' . $set->getJobSetID())?></textarea>
                        </div>
                    </fieldset>
