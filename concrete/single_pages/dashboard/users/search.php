...
<div class="ccm-dashboard-header-buttons btn-group">
	<?php if (Config::get('concrete.user.registration.validate_email') == true && $canActivateUser) { ?>
		...
		<button type="submit" name="task" value="validate" class="btn btn-default"><?php echo t('Mark Email as Valid')?></button>
		<?php } ?>
	<?php } ?>

<?php if ($canActivateUser) { ?>
	<?php if ($user->isActive()) { ?>
		<button type="submit" name="task" value="deactivate" class="btn btn-default"><?php echo t('Deactivate User')?></button>
	<?php } else { ?>
		<button type="submit" name="task" value="activate" class="btn btn-default"><?php echo t('Activate User')?></button>
	<?php } ?>
<?php } ?>

<?php if ($canSignInAsUser) { ?>
	<button type="submit" name="task" value="sudo" class="btn btn-default"><?php echo t('Sign in As User')?></button>
<?php } ?>
...
<section>
	<div class="row">
		...
		<div class="row">
			...
			<div class="col-md-8"><p><span <?php if ($canEditUserName) { ?>data-editable-field-type="xeditable" data-url="<?php echo $view->action('update_username', $user->getUserID())?>" data-type="text" data-name="uName" <?php } ?>><?php echo $user->getUserName()?></span></p></div>
		</div>
		<div class="row">
			<div class="col-md-4"><p><?php echo t('Email Address')?></p></div>
			<div class="col-md-8"><p><span <?php if ($canEditEmail) { ?>data-editable-field-type="xeditable" data-url="<?php echo $view->action('update_email', $user->getUserID())?>"data-type="email" data-name="uEmail"<?php } ?>><?php echo $user->getUserEmail()?></span></p></div>
		</div>
		<div class="row">
			<div class="col-md-4"><p><?php echo t('Password')?></p></div>
			<div class="col-md-8"><p><?php if ($canEditPassword) { ?><a href="#" class="btn btn-xs btn-default" data-button="change-password"><?php echo t('Change')?></a><?php } else { ?>*********<?php } ?></p></div>
		</div>
		<div class="row">
			<div class="col-md-4"><p><?php echo t('Profile Picture')?></p></div>
			<div class="col-md-8"><p>
				<div <?php if ($canEditAvatar) { ?>data-editable-field-type="image" data-editable-field-inline-commands="true" data-url="<?php echo $view->action('update_avatar', $user->getUserID())?>"<?php } ?>>
	                ...
	                <span class="editable-image-wrapper">
	                    ...
	                    <div class="editable-image-display"><?php echo Loader::helper('concrete/avatar')->outputUserAvatar($user)?></div>
					</span>
				</div>
			</p>
			</div>
		...
		<div class="col-md-6">
			...
			<?php if (ENABLE_USER_TIMEZONE) {
				...
				<div class="row">
					...
					<div class="col-md-8"><p><span <?php if ($canEditTimezone) { ?>data-editable-field-type="xeditable" data-source="<?php echo $view->action('get_timezones')?>" data-url="<?php echo $view->action('update_timezone', $user->getUserID())?>" data-type="select2" data-name="uTimezone" data-value="<?php echo h($uTimezone)?>"<?php } ?>><?php echo $dh->getTimezoneDisplayName($uTimezone) ?></span></p></div>
				</div>
			<?php } ?>
			<?php
			...
			<div class="row">
				...
				<div class="col-md-8"><p><span <?php if ($canEditLanguage) { ?>data-editable-field-type="xeditable" data-source="<?php echo $view->action('get_languages')?>" data-url="<?php echo $view->action('update_language', $user->getUserID())?>" data-type="select" data-name="uDefaultLanguage"<?php } ?>><?php echo $user->getUserDefaultLanguage();?></span></p></div>
			</div>
			<?php } ?>
	...
	<div style="display: none">
		<div data-dialog="change-password" class="ccm-ui">
			<form data-dialog-form="change-password" action="<?php echo $view->action('change_password', $user->getUserID())?>">
				<?php echo Loader::helper('validation/token')->output('change_password')?>
