<?php
namespace Application\Block\Carrusel;
...
{
    protected $btTable = 'btCarrusel';
    protected $btExportTables = array('btCarrusel', 'btCarruselEntries');
    ...
    {
        return t("Muestra un Carrusel de fotos");
    }

    public function getBlockTypeName()
    {
        return t("Carrusel Banners");
    }
    ...
    public function getSearchableContent()
    {
        ...
        $db = Database::get();
        $v = array($this->bID);
        $q = 'select * from btCarruselEntries where bID = ?';
        $r = $db->query($q, $v);
        foreach ($r as $row) {
            $content .= $row['title'].' ';
    ...
    public function edit()
    {
        ...
        $this->requireAsset('redactor');
        $db = Database::get();
        $query = $db->GetAll('SELECT * from btCarruselEntries WHERE bID = ? ORDER BY sortOrder', array($this->bID));
        $this->set('rows', $query);
    }
    ...
    {
        $db = Database::get();
        $r = $db->GetAll('SELECT * from btCarruselEntries WHERE bID = ? ORDER BY sortOrder', array($this->bID));
    ...
    public function duplicate($newBID)
    {
        ...
        while ($row = $r->FetchRow()) {
            $db->execute('INSERT INTO btCarruselEntries (bID, fID, linkURL, title, description, target, sortOrder, internalLinkCID) values(?,?,?,?,?,?,?,?)',
                array(
                    $newBID,
                    $row['fID'],
                    $row['linkURL'],
    ...
    {
        $db = Database::get();
        $db->delete('btCarruselEntries', array('bID' => $this->bID));
        parent::delete();
    }
    ...
    public function save($args)
    {
        ...
        $args['slidesToShow'] = isset($args['slidesToShow']) ? intval($args['slidesToShow']) : 3;
        $args['slidesToScroll'] = isset($args['slidesToScroll']) ? intval($args['slidesToScroll']) : 3;
        ...
        $args['modo_center'] = isset($args['modo_center']) ? 1 : 0;
        $args['auto_height'] = isset($args['auto_height']) ? 1 : 0;
        $args['maxWidth'] = isset($args['maxWidth']) ? intval($args['maxWidth']) : 0;

        $db = Database::get();
        $db->execute('DELETE from btCarruselEntries WHERE bID = ?', array($this->bID));
        parent::save($args);
        if (isset($args['sortOrder'])) {
            ...
            while ($i < $count) {
                ...
                $db->execute('INSERT INTO btCarruselEntries (bID, fID, title, description, target, sortOrder, linkURL, internalLinkCID) values(?,?,?,?,?,?,?,?)',
                    array(
                        $this->bID,
                        intval($args['fID'][$i]),
                        $args['title'][$i],
