...
namespace Application\Block\Imageheader;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btContentImageHeader';
    ...
    public function getImageFeatureDetailFileObject()
    {
        ...
        $fID = $db->fetchColumn('select fID from btContentImageHeader where bID = ?', array($this->bID), 0);
        if ($fID) {
            $f = File::getByID($fID);
            if (is_object($f) && !$f->isError()) {
                return $f;
